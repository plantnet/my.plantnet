        ...
        for (project in projects) {
            ...
            if (! is.null(resp)) {
                ...
                if (status == 200 && is.list(data["results"]) && length(data["results"][[1]]) > 0) {
                    ...
                    results_line <- c(results_line_header, c(is_top_1, is_in_top_5), c(rank), results_line)
...
# convert to CSV
csv_data <- "subfolder/ground truth;image;project;is top1;in top5;rank" # headers
for (i in 1: results_limit) {
